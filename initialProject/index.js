...
const fs = require('fs');
const path = require('path'); // Container Object
const server = {}; // Base Directory - Assuming minimal-http-server
// will be accessed  from its own folder.
const baseDir = '/app'; // Create a server
const httpServer = http.createServer((request, response) => {
  try {
    ...
    const responseContentType = getContentType(pathName); // Set the 'Content-Type' in response header
    ...
    response.end('500 - Internal server error: ' + JSON.stringify(error));
  }
}); // Mime Types
const mimeTypes = {
  '.html': 'text/html',
...
const getContentType = (pathName) => {
  ...
  let contentType = 'application/octet-stream'; // Set the contentType based on mime type
